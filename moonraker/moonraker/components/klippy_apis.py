...
class KlippyAPI(APITransport):
    ...
    async def _gcode_start_print(self, web_request: WebRequest) -> str:
        ...
        return await self.start_print(filename, user=user, plateindex = plateindex, need_extract = need_extract)
    ...
    async def start_print(
        ...
        script = f'SDCARD_PRINT_FILE FILENAME="{filename}" PLATEINDEX="{plateindex}"'
